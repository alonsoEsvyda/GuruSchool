<?php
	...
	if (!isset($_POST['Id']) || $_POST['Id']=="") {
		...
		$IdVideo=$GuruApi->TestInput($IdDecode);
			...
			$SqlGetName=$conexion->prepare("SELECT Vc_VideoArchivo FROM G_Videos_Curso WHERE Id_Pk_VideosCurso = ?");
			$SqlGetName->bind_param("i",$IdVideo);
			if ($SqlGetName->execute()) {
				$SqlGetName->store_result();
				$SqlGetName->bind_result($NameVideo);
				$SqlGetName->fetch();
				//Eliminamos el vídeo
				unlink("../../videos_user/".$NameVideo);
					//Eliminamos el Curso
