...
if (!isset($_POST['NameSubCat']) || !isset($_POST['Quanti'])) {
	echo false;
}else{
	...
	$Quantity=$GuruApi->TestInput($_POST['Quanti']);
	$State="Publicado";
	$SqlGetCourses=$conexion->prepare("SELECT Id_Pk_Curso,Int_Fk_IdUsuario,Vc_NombreCurso,Vc_Imagen_Promocional,Int_PrecioCurso,Vc_TipoCurso FROM G_Cursos WHERE Vc_SubCategoria = ? AND Vc_EstadoCurso = ? ORDER BY RAND() DESC LIMIT ? ");
	$SqlGetCourses->bind_param("ssi", $NameSubcat,$State,$Quantity);
